...
function getEvents(events: FindEventDTO[], id: string) {
    return events.map(c => {
        ...
        const tags = c.tags;
        return (
            <Link href={`/home/calendars/${encodeURIComponent(id)}/${encodeURIComponent(c.name)}`} className="w-full min-h-40 max-w-5xl flex sm:flex-row hover:scale-105 transition-transform flex-col bg-white/5 rounded-lg p-4 gap-2" key={c.name}>
